...
vis_miss(merged_AHA[c(Missing_cate, Missing_con)], sort_miss = TRUE) +
  theme_minimal() +
  theme(
  ...
  scale_x_discrete(labels = function(x) {
    # Custom label: variable name on first line, percent missing on second line
    pct <- round(colMeans(is.na(merged_AHA[Missing_cate])) * 100)
    paste0(x, "\n(", pct[x], "%)")
  })
...
# Optional: Add only numbers for |r| ≥ 0.7 from the masked matrix
# This step is only visual (for your clarity, not required)


#corrplot(correlation_matrix, method = "number")

# Identify highly correlated variables (threshold = 0.8)
high_corr_indices <- findCorrelation(correlation_matrix, cutoff = 0.8)
high_corr_vars <- colnames(correlation_matrix)[high_corr_indices]

...
imp <- merged_AHA_clean[imp_data_vars]

imp$PLNTA = as.numeric(log(imp_data$PLNTA))
imp$GFEET = as.numeric(log(imp_data$GFEET))
imp$CEAMT = as.numeric(log(imp_data$CEAMT))
...
plot(imp)      # trace for regression-based
densityplot(imp)
...
###############
# Define your features
features2 <- c('VEM', 'FTMDTF', 'FTRNTF', 'PLNTA', 'GFEET', 'CEAMT', 'VIDVZ', 'PRPM',
               'CHC', 'MAPP1', 'MAPP18', 'MAPP20', 'IINSPT', 'CMRPAY', 'FAMADV',
               'COUTRHOS', 'FITCHOS', 'HLTHSHOS', 'HLTRHOS', 'EMDEPHOS',
               'NUTRPHOS', 'ONCOLHOS', 'PALHOS', 'SOCEHR', 'OUTMTX', 'WFAIPPD', 'COLLCLI',
               'TRAUML90', 'SCNED', 'CLUSTER','Year')
